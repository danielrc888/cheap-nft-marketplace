...
import { createAuction, placeBid, approveBid, getAuctionDetails, getAllBids, getAllAuctions } from "./services/auctionService";
import { generateSignatureMessage, verifySignature } from "./utils/signatureUtils";
import { CreateAuctionParams, CreateBidParams, ApproveBidParams } from "./types/auctionTypes";
...
// Create a bid
app.post("/auction/:auctionId/bid/create", (req: Request<{auctionId: string}, {}, CreateBidParams>, res: Response) => {
    const { auctionId } = req.params;
    const auction = getAuctionDetails(auctionId);
    if (!auction) {
        res.status(404).send("Auction not found.");
...
// Approve a bid
app.post("/auction/:auctionId/bid/approve", (req: Request<{auctionId: string}, {}, ApproveBidParams>, res: Response) => {
    const { auctionId } = req.params;
    const auction = getAuctionDetails(auctionId);
    if (!auction) {
        res.status(404).send("Auction not found.");
