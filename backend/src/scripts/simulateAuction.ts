...
const marketPlaceABI = [{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"nftOwner","type":"address"},{"indexed":true,"internalType":"address","name":"bidder","type":"address"}],"name":"AuctionSettled","type":"event"},{"inputs":[{"internalType":"address","name":"nftAddress","type":"address"},{"internalType":"address","name":"nftOwner","type":"address"},{"internalType":"uint256","name":"nftTokenId","type":"uint256"},{"internalType":"uint256","name":"minPrice","type":"uint256"},{"internalType":"address","name":"erc20Address","type":"address"},{"internalType":"address","name":"bidder","type":"address"},{"internalType":"uint256","name":"bidAmount","type":"uint256"},{"internalType":"bytes","name":"ownerSignature","type":"bytes"},{"internalType":"bytes","name":"bidderSignature","type":"bytes"}],"name":"settleAuction","outputs":[],"stateMutability":"nonpayable","type":"function"}]
...
async function simulateAuction() {
    ...
    const auction = (await axios.post(`${backendUrl}/auction/create`, auctionData)).data
    ...
    const bid = (await axios.post(`${backendUrl}/auction/${auction.id}/bid/create`, createBidBody)).data

    // Approve the bid
    const nftOwnerSignature = await nftOwnerWallet.signingKey.sign(auctionHashBytes).serialized;
    ...
    const approvedAuction = (await axios.post(`${backendUrl}/auction/${auction.id}/bid/approve`, approveBidBody)).data

    // Get the auction signed by the nft owner and bidder
    const marketplaceContract = new ethers.Contract(marketPlaceAddress, marketPlaceABI, nftOwnerWallet);
    // return
    try {
